...
int ads7870_exit(void) {
	...
	printk(KERN_DEBUG "ads7870_exit\n");
...
u16 ads7870_convert(u8 channel) {
	...
	printk(KERN_DEBUG "ads7870_convert\n");

	/*
	 * Start Conversion
	 */
	ads7870_spi_write_reg8(ads7870_spi_device, ADS7870_GAINMUX, ADS7870_GAIN_1X
			| ADS7870_CH_SINGLE_ENDED | (channel & 0x07) | ADS7870_CONVERT);
	 ...
	 */
	i = 0;
	while ((ads7870_spi_read_reg8(ads7870_spi_device, ADS7870_GAINMUX)
			& ADS7870_CONVERT) > 0) {
		i++;
		/*
	...
	if (MODULE_DEBUG)
		printk(KERN_DEBUG "Had to poll ADS7870 %i times before result was ready\n", i);
